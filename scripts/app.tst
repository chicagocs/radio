document.addEventListener('DOMContentLoaded', () => {
    ...
    // NUEVO: Variables para el sistema de actualización adaptativo
    ...
    function playStation() {
        ...
        audioPlayer.play()
            .then(() => { 
                ...
                if (currentStation.service !== 'nrk') {
                    setTimeout(() => startAdaptiveSongInfoUpdates(), 5000);
                }
    ...
    // MEJORADA: Función para actualizar información de SomaFM con verificación de consistencia
        ...
        try {
            ...
            if (data.songs && data.songs.length > 0) {
                ...
                const newTrackInfo = { 
                    ...
                    lastChangeTime: Date.now() // Añadir timestamp de cuándo se detectó el cambio
                };
                
                // Verificación mejorada de cambio de canción
                const isNewTrack = !currentTrackInfo || 
                                  currentTrackInfo.title !== newTrackInfo.title || 
                                  currentTrackInfo.artist !== newTrackInfo.artist;
                
                if (isNewTrack) {
                    // Verificación adicional: ¿cuánto tiempo ha pasado desde el último cambio?
                    if (currentTrackInfo && currentTrackInfo.lastChangeTime) {
                        const timeSinceLastChange = Date.now() - currentTrackInfo.lastChangeTime;
                        
                        // Si el cambio es muy rápido (<10 segundos), podría ser un error de la API
                        if (timeSinceLastChange < 10000) {
                            console.warn('Cambio de canción detectado muy rápidamente, posible error de API');
                            // Esperar una confirmación adicional antes de actualizar
                            pendingSongVerification = newTrackInfo;
                            setTimeout(() => verifySongChange(newTrackInfo), 3000);
                            return;
                        }
                    }
                    ...
                    // Ajustar la frecuencia de actualización después de un cambio
                    updateFrequency = 5000; // Restablecer a 5 segundos después de un cambio
                }
            } else { 
                resetUI(); 
            }
        } catch (error) { 
            console.error('Error al obtener info de SomaFM:', error); 
            resetUI(); 
    ...
    // NUEVO: Función para verificar si un cambio de canción es legítimo
    async function verifySongChange(potentialNewTrack) {
        // Si ya hay una verificación en proceso, cancelar la anterior
        if (pendingSongVerification && 
            pendingSongVerification.title === potentialNewTrack.title && 
            pendingSongVerification.artist === potentialNewTrack.artist) {
            return;
        }
        
        try {
            // Permitir una llamada inmediata a la API para verificación
            apiCallTracker.somaFM.lastCall = 0;
            const response = await fetch(`https://api.somafm.com/songs/${currentStation.id}.json`);
            const data = await response.json();
            ...
            if (data.songs && data.songs.length > 0) {
                ...
                // Si la verificación confirma el cambio, actualizar
                if (verificationTrack.title === potentialNewTrack.title && 
                    verificationTrack.artist === potentialNewTrack.artist) {
                    
                    resetCountdown(); 
                    resetAlbumDetails(); 
                    currentTrackInfo = {...potentialNewTrack, lastChangeTime: Date.now()};
                    lastSongChangeTime = Date.now();
                    updateUIWithTrackInfo(currentTrackInfo); 
                    resetAlbumCover();
                    trackStartTime = currentTrackInfo.date ? currentTrackInfo.date * 1000 : Date.now();
                    
                    await fetchSongDetails(currentTrackInfo.artist, currentTrackInfo.title, currentTrackInfo.album);
                    
                    // Ajustar la frecuencia de actualización después de una verificación exitosa
                    updateFrequency = 5000;
                } else {
                    console.log('Verificación de cambio de canción falló, manteniendo canción actual');
                }
            }
        } catch (error) {
            console.error('Error en verificación de cambio de canción:', error);
        }
        
        pendingSongVerification = null;
    }
    ...
    // MEJORADA: Nueva versión optimizada con requestAnimationFrame y detección de fin de canción
    function startCountdown() {
        ...
        function updateTimer() {
            ...
            if (remaining > 0) {
                ...
                // La canción ha terminado - forzar una actualización inmediata
                countdownTimer.textContent = '00:00';
                countdownTimer.classList.remove('ending');
                
                // Forzar una consulta inmediata a la API
                if (currentStation && currentStation.service === 'somafm') {
                    // Permitir una llamada inmediata a la API
                    apiCallTracker.somaFM.lastCall = 0;
                    updateSomaFmInfo();
                } else if (currentStation && currentStation.service === 'nrk') {
                    stopBtn.click();
                } else {
                    updateSongInfo();
                }
    ...
    // NUEVO: Sistema de actualización adaptativo
    function startAdaptiveSongInfoUpdates() {
        updateSongInfo();
        
        function updateWithAdaptiveFrequency() {
            updateSongInfo();
            
            // Ajustar la frecuencia de actualización según los cambios detectados
            if (!currentTrackInfo || !lastSongChangeTime) {
                // Si no hay información de canción, aumentar la frecuencia
                updateFrequency = Math.max(updateFrequency * 0.9, 3000); // Mínimo 3 segundos
            } else {
                const timeSinceChange = Date.now() - lastSongChangeTime;
                
                // Si ha pasado mucho tiempo desde el último cambio, reducir frecuencia
                if (timeSinceChange > 120000) { // 2 minutos
                    updateFrequency = Math.min(updateFrequency * 1.2, 30000); // Máximo 30 segundos
                } else if (timeSinceChange > 60000) { // 1 minuto
                    updateFrequency = Math.min(updateFrequency * 1.1, 20000); // Máximo 20 segundos
                } else {
                    // Si el cambio fue reciente, mantener frecuencia alta
                    updateFrequency = Math.max(updateFrequency * 0.95, 5000); // Mínimo 5 segundos
                }
            }
            
            if (updateInterval) clearTimeout(updateInterval);
            updateInterval = setTimeout(updateWithAdaptiveFrequency, updateFrequency);
        }
    ...
    function setupAudioChangeDetection() {
        ...
        try {
            ...
            let lastAudioChangeTime = 0;
            
            function detectAudioChanges() {
                if (!isPlaying || !audioChangeDetectionActive) {
                    requestAnimationFrame(detectAudioChanges);
                    return;
                }
                ...
                // Detectar cambios bruscos en el nivel de audio
                const levelChange = Math.abs(average - previousAudioLevel);
                
                if (levelChange > 30) { // Umbral ajustable
                    consecutiveChanges++;
                    
                    // Si detectamos varios cambios consecutivos, podría indicar un cambio de canción
                    if (consecutiveChanges >= 3) {
                        const now = Date.now();
                        
                        // Evitar detecciones demasiado frecuentes
                        if (now - lastAudioChangeTime > 10000) { // Mínimo 10 segundos entre detecciones
                            console.log('Posible cambio de canción detectado por análisis de audio');
                            consecutiveChanges = 0;
                            lastAudioChangeTime = now;
                            
                            // Forzar una consulta a la API para verificar
                            if (canMakeApiCall('somaFM') && currentStation.service === 'somafm') {
                                // Permitir una llamada inmediata a la API
                                apiCallTracker.somaFM.lastCall = 0;
                                updateSomaFmInfo();
                            }
                ...
                previousAudioLevel = average;
                requestAnimationFrame(detectAudioChanges);
            }
            
            audioChangeDetectionActive = true;
            detectAudioChanges();
    ...
    function stopSongInfoUpdates() {
        ...
        // Detener la detección de cambios de audio
        audioChangeDetectionActive = false;
    }
    ...
    audioPlayer.addEventListener('ended', () => { 
        ...
        // Detener la detección de cambios de audio cuando termina la reproducción
        audioChangeDetectionActive = false;
    });
    ...
    if ('mediaSession' in navigator) {
        ...
        navigator.mediaSession.setActionHandler('pause', () => {
            if (isPlaying) {
                ...
                // Detener la detección de cambios de audio al pausar la reproducción
                audioChangeDetectionActive = false;
            }
        });
    }
