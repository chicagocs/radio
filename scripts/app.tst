...
function startCountdown() {
  ...
  // v3.7.6: Estado reactivo para evitar reimpresión y parpadeo
  let lastRenderedElapsed = null;
  ...
  function updateTimer() {
    ...
    const elapsedSeconds = (now - trackStartTime) / 1000;
    const clampedElapsed = Math.max(0, elapsedSeconds);

    // === Actualizar tiempo restante (countdownTimer) ===
    let remainingSeconds;
    if (trackDuration > 0) {
      remainingSeconds = Math.max(0, trackDuration - clampedElapsed);
    } else {
      remainingSeconds = clampedElapsed; // modo "elapsed" si no hay duración
    }

    const remMin = Math.floor(remainingSeconds / 60);
    const remSec = Math.floor(remainingSeconds % 60);
    const newElapsedText = `${String(remMin).padStart(2, '0')}:${String(remSec).padStart(2, '0')}`;

    if (newElapsedText !== lastRenderedElapsed) {
      lastRenderedElapsed = newElapsedText;
      countdownTimer.textContent = newElapsedText;

      // Clase visual para últimos 10 segundos
      if (trackDuration > 0 && remainingSeconds < 10) {
    ...
    // === Actualizar duración total (totalDuration) ===
    let newTotalText;
    if (trackDuration > 0) {
      const totMin = Math.floor(trackDuration / 60);
    ...
    if (newTotalText !== lastRenderedTotal) {
      lastRenderedTotal = newTotalText;
      totalDuration.textContent = newTotalText;
    }

    // === Transición automática al final ===
    if (trackDuration > 0 && remainingSeconds <= 0) {
      countdownTimer.textContent = '00:00';
      countdownTimer.classList.remove('ending');

      if (cachedNextTrack && ['somafm', 'radioparadise'].includes(currentStation?.service)) {
        loadTrackFromCache(cachedNextTrack);
        return;
      } else {
        updateSongInfo(true);
    ...
    // === Continuar animación ===
    if ((trackDuration > 0 && remainingSeconds > 0) || trackDuration === 0) {
      animationFrameId = requestAnimationFrame(updateTimer);
    } else if (currentStation?.service === 'nrk') {
      stopBtn.click();
    }
  }
