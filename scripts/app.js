...
// --- NUEVAS VARIABLES GLOBALES PARA COUNTDOWN MEJORADO ---
...
// 4. FUNCIÓN updateTimer REDISEÑADA CON MEJOR LÓGICA
function updateTimer() {
    ...
    // CASO 2: Tenemos duración - calcular tiempo restante
    remaining = trackDuration - elapsed;

    // CASO 2A: Tiempo restante positivo (cuenta regresiva normal)
    if (remaining > 0) {
        const mins = Math.floor(remaining / 60);
        const secs = Math.floor(remaining % 60);
        displayText = `${String(mins).padStart(2,'0')}:${String(secs).padStart(2,'0')}`;
        
        // Efecto visual en últimos 10 segundos
        if (remaining < 10) {
        ...
        // Trigger de verificación agresiva en últimos 5 segundos
        if (remaining <= 5 && !rapidCheckInterval && currentStation?.service === 'somafm') {
            rapidCheckInterval = setInterval(() => {
                updateSongInfo(true);
            }, 1000); // Verificar cada segundo en el final
        }
    }
    // CASO 2B: Tiempo pasado (la canción debería haber cambiado)
    else {
        const overtime = Math.abs(remaining);
        const mins = Math.floor(overtime / 60);
        const secs = Math.floor(overtime % 60);
        displayText = `+${String(mins).padStart(2,'0')}:${String(secs).padStart(2,'0')}`;
        
        // Mantener el efecto de parpadeo
        countdownTimer.classList.add('ending');
        
        // Trigger de actualización inmediata si llevamos más de 2 segundos en overtime
        if (overtime > 2 && overtime < 3 && !isUpdatingSongInfo) {
            console.log('Overtime detectado, forzando actualización...');
            updateSongInfo(true);
        }
...
async function updateSomaFmInfo(bypassRateLimit = false) {
    ...
    try {
        ...
        if (data.songs && data.songs.length > 0) {
            ...
            // Crear ID único para detectar cambios reales de canción (evita cambios si el radio repite datos)
            const newTrackId = `${newTrack.artist}-${newTrack.title}`;
            const isNewTrack = lastKnownTrackId !== newTrackId;
            
            if (isNewTrack) {
                console.log('🎵 Nueva canción detectada:', newTrack.title);
...
// 6. MEJORAS EN updateRadioParadiseInfo - DETECCIÓN DE CAMBIO
    ...
    try {
        ...
        if (isrc) {
            try {
                ...
                // Si falla la búsqueda por ISRC (ej código inválido), caemos al bloque siguiente
