document.addEventListener('DOMContentLoaded', () => {
    ...
    // MEJORADO: Función para controlar llamadas a API con opción de forzar
    function canMakeApiCall(service, force = false) {
        const now = Date.now();
        // Si se fuerza la llamada, reducir el intervalo mínimo
        const minInterval = force ? 
            apiCallTracker[service].minInterval / 2 : 
            apiCallTracker[service].minInterval;
        
        if (now - apiCallTracker[service].lastCall >= minInterval) {
    ...
    // MEJORADO: Función para actualizar información de la canción con opción de forzar
    async function updateSongInfo(forceUpdate = false) {
        if (!currentStation || !currentStation.service) return;
        
        // Permitir llamadas más frecuentes si se fuerza la actualización
        if (currentStation.service === 'somafm') {
            if (forceUpdate || canMakeApiCall('somaFM')) { 
                await updateSomaFmInfo(); 
            }
        } else if (currentStation.service === 'radioparadise') {
            if (forceUpdate || canMakeApiCall('radioParadise')) { 
    ...
    // MEJORADO: Función de cuenta regresiva con mayor precisión
    function startCountdown() {
        ...
        if (trackDuration > 0 && trackStartTime > 0) {
            ...
            // Actualizar cada 500ms en lugar de 1000ms para mayor precisión
    ...
    // MEJORADO: Reducir el intervalo de actualización de 30 a 15 segundos
    function startSongInfoUpdates() {
        updateSongInfo();
        if (updateInterval) clearInterval(updateInterval);
        // Reducir el intervalo de 30 a 15 segundos para mayor precisión
        updateInterval = setInterval(updateSongInfo, 15000);
    }
