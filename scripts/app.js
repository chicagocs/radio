document.addEventListener('DOMContentLoaded', () => {
    ...
    // NUEVO: Variable para rastrear tooltips activos
    ...
    // MODIFICADO: Función showNotification mejorada con verificación de nulidad
    function showNotification(message) {
        // Verificar si el elemento notification existe antes de intentar usarlo
        if (!notification) {
...
function updateAlbumDetailsWithSpotifyData(data) {
    ...
    function updateUIWithTrackInfo(trackInfo) {
        ...
        if (songArtist) songArtist.textContent = trackInfo.artist;
        if (songAlbum) songAlbum.textContent = trackInfo.album ? `(${trackInfo.album})` : '';
        updateShareButtonVisibility();
    }

    function resetUI() {
        if (connectionManager.isReconnecting) { return; }
        if (songTitle) songTitle.textContent = 'Reproduciendo...';
    ...
    // MODIFICADO: Función resetAlbumDetails mejorada para evitar llamadas innecesarias
    function resetAlbumDetails() {
        console.log('🔴 resetAlbumDetails LLAMADA - BORRANDO TODO');
        
        // Limpiar tooltips activos
        cleanupTooltips();
        
        // Solo resetear si los elementos existen
    ...
    volumeSlider.addEventListener('input', function() {
        ...
        if (this.value == 0) { 
            if (volumeIcon) volumeIcon.classList.add('muted'); 
            isMuted = true; 
        } else { 
            if (volumeIcon) volumeIcon.classList.remove('muted'); 
            isMuted = false; 
    ...
    const connectionManager = {
        ...
        attemptReconnect() {
            ...
            if (this.reconnectAttempts >= this.maxReconnectAttempts) {
                ...
                if (songTitle) songTitle.textContent = 'Error de conexión: no se pudo restaurar';
                if (songArtist) songArtist.textContent = 'Intenta reproducir manualmente';
                this.stop();
                return;
            }
            ...
            if (songTitle) songTitle.textContent = `Intentando reconectar... (${this.reconnectAttempts}/${this.maxReconnectAttempts})`;
            if (songArtist) songArtist.textContent = `Próximo intento en ${Math.ceil(delay / 1000)}s.`;
            if (songAlbum) songAlbum.textContent = '';
    ...
    if (shareWhatsApp) {
        shareWhatsApp.addEventListener('click', () => {
            ...
            const artist = songArtist ? songArtist.textContent : '';
    ...
    if (volumeSlider) updateVolumeIconPosition();
