// app.js - v3.5.0 (Arquitectura de Eventos Centralizada y Reconexión Forzada)
...
async function joinStation(stationId) {
    // FIX: Eliminamos la protección early return.
    // Esto permite forzar una reconexión/renovación del canal
    // si el usuario re-selecciona la estación actual (útil tras desconexiones).
    // La limpieza leaveStation se ejecuta de forma segura.

    // Limpieza del canal anterior (si existe y es diferente, o forzada si queremos renovar)
    if (currentChannel) {
        await leaveStation(currentStationId);
    }
    
    // Asignar ID global inmediatamente
...
async function leaveStation(stationId) {
    if (currentChannel) {
        try {
            ...
            // Ignorar errores de socket cerrado durante cambios rápidos
...
// ==========================================================================
// SELECTOR PERSONALIZADO (ACTUALIZADO V3.5.0)
// ==========================================================================
class CustomSelect {
...
async function loadStations() {
    try {
        ...
        // FIX: Conectar CustomSelect al evento change del original para auto-actualizar UI con el teclado
        stationSelect.addEventListener('change', () => {
            customSelect.updateTriggerText();
            customSelect.updateSelectedOption();
            // Cerrar el dropdown al seleccionar por teclado para mejor UX
            if (customSelect.customSelectWrapper.classList.contains('open')) {
                customSelect.close();
            }
        });
...
async function playStation() {
    ...
    // FIX CRÍTICO 1: Asegurar limpieza y setup del canal
    ...
    try {
        ...
        // FIX: No llamamos handlePlaybackError para abortos manuales
...
// FIX: Handler de teclado limpio. Lógica delegada en eventos.
document.addEventListener('keydown', function(event) {
    if (!document.querySelector('.custom-select-wrapper.open') &&
        ...
        if (matches.length > 0) {
            ...
            // Eliminamos actualizaciones manuales de UI.
            // El CustomSelect ahora escucha el evento 'change' del select nativo
            // y se encarga de actualizar el texto y la selección visual.
