...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        // Elementos del encabezado del reproductor
        ...
        let showOnlyFavorites = false; // Control del filtro de favoritos

        let wasPlayingBeforeFocusLoss = false;
        let pageFocusCheckInterval = null;
        let lastAudioCurrentTime = 0; // Corregido: para checkAudioContext
        ...
        // Constante para activar verificación rápida en SomaFM
        ...
        function updateShareButtonVisibility() {
            ...
            const blacklist = ['a sonar', 'Conectando...', 'Seleccionar estación', 'A sonar', 'Reproduciendo...', 'Error de reproducción', 'Reconectando...'];
            
            if (title && artist && !blacklist.includes(title) && artist !== '') {
        ...
        function checkAudioContext() {
            // BUG FIX: Lógica corregida para comparar tiempos de reproducción reales
            if (!audioPlayer.paused && isPlaying) {
                const currentTime = audioPlayer.currentTime;
                // Si el tiempo no cambia en 3 segundos, asumimos que está atascado
        ...
        function updateFavoriteButtonUI(stationId, isFavorite) {
            ...
            if (btn) {
                const option = btn.closest('.custom-option');
                if (!option) return;
                
                const stationName = option.querySelector('.custom-option-name').textContent;
        ...
        class CustomSelect {
            ...
            createCustomOption(option) {
                ...
                favoriteBtn.addEventListener('click', (e) => {
                    ...
                    if (isFavorite) removeFavorite(stationId);
            ...
            updateSelectedOption() {
                ...
                const customOptions = this.customOptions.querySelectorAll('.custom-option');
                customOptions.forEach(option => {
                    if (option.dataset.value === selectedValue) option.classList.add('selected');
        ...
        // ==========================================================================
        // FUNCIONES OPTIMIZADAS PARA PORTADA
        // ==========================================================================
        ...
        // ==========================================================================
        // INTEGRACIÓN BOTÓN FILTRO FAVORITOS
        // ==========================================================================
        if (filterToggleStar) {
        ...
        function playStation() {
            ...
            // Limpieza completa de intervalos previos
            ...
            audioPlayer.play()
                .then(() => {
                    ...
                    setTimeout(() => {
                        if (isPlaying) startPlaybackChecks();
                    }, 2000);
                })
                .catch(error => {
                    handlePlaybackError();
        ...
        async function fetchSongDetails(artist, title, album) {
            if (!artist || !title || typeof artist !== 'string' || typeof title !== 'string') return;
        ...
        function updateAlbumDetailsWithSpotifyData(data) {
            if (releaseDateElement = document.getElementById('releaseDate')) {
        ...
        function startCountdown() {
            ...
            // Lógica para activar rapidCheck en SomaFM
        ...
        document.addEventListener('keydown', function(event) {
            if (!document.querySelector('.custom-select-wrapper.open') &&
                ...
                const matches = [];
                customOptions.forEach(option => {
                    // FIX: Solo considerar opciones visibles
