...
// Permitir reconexión incluso si es la misma estación (previene errores tras pausa/stop)
// No retornamos temprano; en su lugar, limpiamos el canal anterior si existe y es diferente
if (currentChannel && currentStationId !== stationId) {
await leaveStation(currentStationId);
}
// Si ya estamos en la misma estación y el canal ya está activo, no necesitamos reconectar
if (currentStationId === stationId && currentChannel) {
return;
}

// Asignar ID inmediatamente para evitar race conditions
...
if (st) {
currentStation = st;
currentStationId = st.id; // 👈 Aseguramos que currentStationId esté sincronizado
stationName.textContent = st.service === 'radioparadise' ? st.name.split(' - ')[1] || st.name : st.name;
showWelcomeScreen();
playStation();
}
...
// ==========================================================================
// MANEJADOR DE ATAJO DE TECLAS (CORREGIDO)
// ==========================================================================
...
// 👉 NO HACEMOS RETURN SI ES LA MISMA ESTACIÓN
// 👉 DEJAMOS QUE EL EVENTO 'change' MANEJE TODO
stationSelect.value = id;
stationSelect.dispatchEvent(new Event('change'));

// 👇 ELIMINAMOS LA ACTUALIZACIÓN MANUAL DEL UI
// TODO se gestiona a través del listener de 'change' del <select>
}
}
});
