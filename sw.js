// ==========================================================================
// SERVICE WORKER - v3.2.8 (Standard Compliant)
// ==========================================================================
...
self.addEventListener('fetch', event => {
  ...
  // Ignorar solicitudes de estadísticas/analytics
  if (url.hostname.includes('stats.max.com') || url.hostname.includes('stats.tramax.com.ar')) {
    return;
  }

  // Estrategia para navegación (Network First para HTML)
  ...
  // Estrategia para Activos Estáticos (Stale While Revalidate)
  if (STATIC_ASSETS.includes(url.pathname)) {
    event.respondWith(staleWhileRevalidate(event, STATIC_CACHE));
    return;
  }

  // Estrategia para APIs externas (Network First con TTL)
...
// MANEJO DE MENSAJES (Skip Waiting & Debug)
...
async function networkFirstWithTTL(event, cacheName) {
  ...
  // Verificar si el caché es válido basado en TTL personalizado
  if (cached) {
  ...
  try {
    ...
    if (response.ok) {
      // Clonar headers para añadir metadatos de tiempo
...
async function freeUpCacheSpace(cacheName, request) {
  ...
  // Ordenar entradas por antigüedad (LRU)
  ...
  // Eliminar la entrada más antigua que no sea la actual
  for (const entry of entries) {
    ...
    try {
      ...
      console.debug(`[SW] Entrada eliminada por LRU:`, entry.request.url);
      return true;
    } catch (err) {
...
async function getCacheReport(cacheNames) {
  ...
  for (const name of cacheNames) {
    ...
    report[name] = {
      ...
      // Estimación básica, ya que la API no da tamaño directo sin iterar
      // Para precisión se requeriría leer todos los bodies, lo cual es costoso
    };
  }
  return report;
}
