...
function handleOptions() {
  return new Response(null, { status: 200, headers: allHeaders });
}
...
async function handleSpotifyRequest(request, env) {
  try {
    ...
    if (searchData && searchData.tracks.items.length > 0) {
      ...
      return new Response(JSON.stringify(responseData), { status: 200, headers: { ...allHeaders, "Content-Type": "application/json" } });
    }

    // También añadimos el campo nulo en la respuesta de error para consistencia
    return new Response(JSON.stringify({ imageUrl: null, release_date: null, label: null, genres: [], duration: 0, totalTracks: null, totalAlbumDuration: 0, trackNumber: null, albumTypeDescription: null }), { status: 404, headers: { ...allHeaders, "Content-Type": "application/json" } });

  } catch (error) {
    console.error("Error en el worker de Spotify:", error);
    return new Response(JSON.stringify({ error: "Error interno del servidor", details: error.message }), { status: 500, headers: { ...allHeaders, "Content-Type": "application/json" } });
  }
}
...
async function handleRadioParadiseRequest(request) {
  ...
  try {
    ...
    // *** CAMBIO CLAVE: Creamos una nueva respuesta copiando los encabezados y el cuerpo ***
    // En lugar de modificar la respuesta original, creamos una nueva para tener control total.
    // No se pueden modificar directamente los encabezados de una respuesta inmutable.
    const modifiedResponse = new Response(apiResponse.body, {
      status: apiResponse.status,
      statusText: apiResponse.statusText,
      // Combinamos los encabezados de la API original con nuestros encabezados de seguridad y CORS.
      // Nuestros encabezados tienen prioridad si hay colisiones.
      headers: { ...apiResponse.headers, ...allHeaders }
    });
